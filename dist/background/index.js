...
const handleConfigRequest = async (tabId) => {
...
onRuntimeMessage("gesture/action", async (message, sender) => {
...
onRuntimeMessage("config/request", async (_message, sender) => {
    const tabId = sender.tab?.id;
    if (tabId === undefined) {
...
onRuntimeMessage("gesture/triggered", async (message) => {
...
const isContentActive = (tabId) => new Promise((resolve) => {
    try {
...
const ensureContentActive = async (tabId) => {
...
const isRestrictedUrl = (url) => /^(chrome|edge|about|file|devtools):/i.test(url);
// Checks whether we have host permission for the origin.
const hasHostPermission = (pattern) => new Promise((resolve) => {
    try {
...
const requestHostPermission = (pattern) => new Promise((resolve) => {
    try {
